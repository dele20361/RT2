...
STEPS = 1
MAX_RECURSION_DEPTH = 3 # Para que tmb se refleje lo que está atrás.
...
class Raytracer(object):
    ...
    def cast_ray(self, orig, dir, sceneObj = None, recursion = 0):
        ...
        if material.matType == OPAQUE:
            ...
            outside = np.dot(dir, intersect.normal) < 0 # Para ver si el rayo viene de adentro o de afuera.
            bias = intersect.normal * 0.001 # Distancia entre el material y el rayo que general
            reflect = reflectVector(intersect.normal, np.array(dir) * -1) # Vector de reflexión
            
            # Reflejo de la esfera
            reflectOrig = np.add(intersect.point, bias) if outside else np.subtract(intersect.point, bias)
            # if outside: Para que esté encima
            # if inside: Para que esté adentro de la esfera. Mueve el origen del reflejo
            ...
            if kr < 1:
                # Si hay refracción, calculo la refracción
